...
class CytometerDataset(Dataset):
    ...
    """
    def __init__(self, file_idxs=range(1,5), time_window=1, max_samples=None):
        super().__init__()

        paths = [f"{BASE_PATH}/{ab}{i}.npy" for i in file_idxs for ab in "AB"]
        self.time_window = time_window

        # get file lengths according to max_samples
        lengths = [len(np.load(p, allow_pickle=True)) for p in paths]
        if max_samples is not None:
            if isinstance(max_samples, float):
                lengths = [int(e * max_samples) for e in lengths]
            elif isinstance(max_samples, int):
                lengths = [max_samples for _ in lengths]
            else:
                raise ValueError('invalid `max_samples`')

        # each event contains events from a 1e4us=10ms
        self.events = np.concatenate(
            [np.load(p, allow_pickle=True)[:lengths[i]] for i, p in enumerate(paths)]
        )
